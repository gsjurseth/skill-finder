...
We deliberately do NOT depend on ``google-cloud-storage`` --
we keep ``requests.txt`` to four runtime packages.
Instead we issue a single POST against the GCS JSON upload API
with an ADC bearer token from ``google.auth.default()``. The
endpoint shape is:
...
def _credentials():
    ...
    Scope matches the other call sites in this repo
    (skill-finder's ``_creds()``). The GCS upload endpoint
    accepts the cloud-platform scope; using a narrower devstorage
    scope here would diverge from the other Google call sites and
    add a scope-mismatch failure mode for no benefit."""
    import google.auth
