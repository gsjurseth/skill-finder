"""Shared HTTP retry helper for the skill-finder pipeline.

Provides ``http_get_retry`` and ``http_post_retry`` -- both wrap
``requests.{get,post}`` with the locked policy:
...
Hyrum's Law contract line

    [skill-finder] transient failure (HTTP <code>); retry 1/1 after <N>ms

is owned by the caller (``find_install.py``). To let the
caller log retries without polling, both verbs accept an optional
...
shells over it. The two verbs "differ only in HTTP verb."
...
def _request_with_retry(
    ...
    **kwargs: Any,
    ...
    if 500 <= resp.status_code < 600:
        ...
        # the contract line lives in find_install.py.
...
def http_get_retry(
    ...
    **kwargs: Any,
    ...
    """GET ``url`` with the retry policy. See module doc.
...
def http_post_retry(
    ...
    **kwargs: Any,
    ...
    """POST ``url`` with the retry policy. Used by
