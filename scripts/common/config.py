...
Why a config file at all: agent runtimes (Antigravity, Gemini
CLI, OpenCode) inherit their env vars from the shell that
launched them. If the operator launches the IDE before sourcing
the env, the agent's bash subprocesses see no env vars -- even
if the env was set up later. Writing a persistent config file
once breaks that coupling: any process can read the config
regardless of when or where the runtime was launched.
...
def keyword_overlap_threshold(default: int = 1) -> int:
    ...
    Parsing errors fall through to `default` (same as the
    contract in find_install.py)."""
