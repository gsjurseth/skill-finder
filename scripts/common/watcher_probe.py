...
Honest disclosure: the current ENABLED check is
degenerate -- the probe directory always survives because the
probe itself created it. The real value of this function today
is (a) the env-var presence signal and (b) the OSError catch
that makes UNDETECTABLE reachable for the failure-line surface.
A future improvement (out of scope for the demo) would replace
the directory-presence check with a real watcher-response check
...
name: __probe__
description: Internal probe written by skill-finder. Safe to delete.
...
def _env_enabled() -> bool:
    """Per Goldfish D the watcher is opt-in via
