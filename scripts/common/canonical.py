...
The sign side (``sign_skill.py``) and the verify side
(``skill-finder/scripts/find_install.py``) both call
``canonicalize`` to derive the byte string that the ed25519
signature covers. Byte-identical output across both call sites is
a correctness pre-requisite: any drift causes every install to
