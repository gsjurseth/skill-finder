"""IAM pre-flight library for the skill-finder install pipeline.

The runtime_iam field is locked to GCP dot-form strings
(e.g. ``apigee.proxies.list``). This library is called from
``find_install.py`` after manifest signature + schema +
attribute cross-check pass and BEFORE the zip download. The
purpose is to fail fast: if the caller does not hold the
permissions the skill needs at runtime, the install is aborted
...
caller (``find_install.py``) is the SOLE producer of the
Hyrum's Law contract lines

    [skill-finder] IAM pre-flight: OK (<perms>: granted)
    [skill-finder] IAM pre-flight: FAILED — <perm> not granted ...
    [skill-finder] IAM pre-flight: FAILED — HTTP <code> <reason> ...
    [skill-finder] IAM pre-flight: FAILED — HTTP <code> non-JSON body ...
    [skill-finder] IAM pre-flight: skipped (no runtime_iam declared)

so that the ``[skill-finder]`` prefix and the exact wording of the
contract are owned by one module. This library produces structured
data; the caller produces strings.
...
# parent package; bare `common.*` resolves via the
# sys.path.insert in find_install.py at module load. Dev/test
# layout uses `scripts.common.*` via tests/conftest.py.
try:
    from common import http_retry  # production .skill zip layout
except ImportError:
    from scripts.common import http_retry  # dev/test layout

# Uniform OAuth scope: every google.auth.default call in
# this codebase requests exactly this scope so there is no
# silent per-helper drift.
...
class IamPreflightResult:
    ...
    * ``"NON_JSON"`` -- the response body was not valid JSON
    ...
    input -- this matches the contract wording that lists
    missing permissions in the order the operator declared them
    in the manifest's ``runtime_iam`` list.
    """
...
def _creds() -> tuple[object, object]:
    ...
    forbidden. The refresh-before-return invariant was added after
    the real-infra demo run discovered that a fresh
...
def iam_preflight(
    ...
    an ``IamPreflightResult`` status. The caller
    (``find_install.py``) reads the result and emits the
    contract line.
    ...
    * Response body is not valid JSON ->
    ...
    # POST through the shared retry helper -- one retry
    # on 5xx with jittered 200-400 ms backoff. We do not pass
    # on_retry: the transient-failure contract line is the
    # caller's responsibility, and we are a pure data API.
    try:
