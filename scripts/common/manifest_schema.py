...
API hub), and by ``skill-finder/scripts/find_install.py`` (after
...
1. **Dependency surface.** Adding jsonschema or pydantic would
   inflate ``requirements.txt`` past the four-package runtime
   budget; both have larger transitive trees than the four
   current deps combined.
2. **Error messages.** A hand-rolled validator produces
   contract-style failure messages naturally; jsonschema's
   default ValidationError messages are noisier than the
   Hyrum's Law surface allows.
...
# Schema field regexes. Compiled once at import time.
...
def validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    # The 100-item cap is NOT in the documented schema (which
    # leaves the list unbounded). We add it here as a defensive
    # upper bound: an adversarial manifest with 10k IAM strings
    # would force the install-time pre-flight to make 10k
    # testIamPermissions calls. 100 is generously above the
    # legitimate ceiling (apigee-policy-top10 uses 3 permissions;
    # a hypothetical full-Apigee management skill might need
    # 30-40).
